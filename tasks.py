...
def try_load_toml(filepath: Path, key: Optional[str] = None) -> Optional[dict]:
    if not filepath.exists():
        return None

    try:
        result = tomlkit.loads(filepath.read_text()).unwrap()
        return result.get(key, {}) if key else result
    except Exception:
        return None


def find_toml_config(key: Optional[str] = None) -> dict:
    # 1. .toml
    # 2. config.toml
    # 3. default.toml
    return (
        try_load_toml(Path(".toml"), key=key)
        or try_load_toml(Path("config.toml"), key=key)
        or try_load_toml(Path("default.toml"), key=key)
        or {}
    )


@task()
def generate_cors(_: Context) -> Optional[str]:
    if config := find_toml_config(key="cors"):
        cors = " ".join(
            [f"https://*.{_} https://{_}" for _ in config.get("domains", [])]
        )
        edwh.set_env_value(Path(".env"), "CORS", cors)
...
def setup(c):
    ...
    edwh.tasks.check_env(
        key="TRAEFIK_CERTIFICATE_EMAIL",
        default="",
        comment="Required email for letsencrypt",
    )
    edwh.tasks.check_env(
        key="TRAEFIK_PILOT_TOKEN",
        default="default",
        comment="Optional pilot token for traefik",
    )
    domain = edwh.tasks.check_env(
        key="HOSTING_DOMAIN",
        default="dockers.local",
...
def mk_certificate(c, domain):
    ...
    if not Path("root_cert/ca.cert.pem").exists():
        c.run(
            "openssl req -x509 -new -nodes -key root_cert/ca.private.pem -days 10000 -out root_cert/ca.cert.pem "
    ...
    if not Path("server/csr.pem").exists():
        c.run(
            f'openssl req -new -key server/private.pem -out server/csr.pem -subj "/C=NL/O=EDWH dev/CN=*.{domain}"'
        )
    else:
        print("already exists")
    ...
    c.run(
        "openssl x509 -outform der -in root_cert/ca.cert.pem -out client/ca.cert.der-format.crt"
    )
